...
| 目录 | 说明 | 状态 |
|------|------|------|
| [`books/book-1/`](books/book-1/) | GitHub 版本发布与云原生 GitOps 实战教程 | ✅ v0.02 已发布 |
| [`books/book-2/`](books/book-2/) | 第二本书（模板骨架） | 🔧 撰写中 |
| `books/book-3/` | 第三本书（待创建） | ⏳ 规划中 |
...
| 工作流 | 触发条件 | 作用 |
|--------|----------|------|
| **Validate** | push / PR / 手动 | 校验所有书稿完整性，确保 PDF/MD/SHA256 一致 |
...
```bash
# 1. 更新 VERSION 文件
echo "0.03" > books/book-1/VERSION
...
# 5. 提交并推送
git add .
git commit -m "release: book-1 v0.03"
git push origin main

# 6. 打 Tag（命名规则：{book目录名}-v{版本号}）
git tag -a book-1-v0.03 -m "book-1: Release v0.03"
git push origin book-1-v0.03
#    ↑ 推送 Tag 后 Release 工作流自动运行
```

> **Tag 命名规则**：`{book目录名}-v{版本号}`，例如 `book-1-v0.02`、`book-2-v0.01`。  
> 工作流会自动解析出 book 目录和版本号，找到对应的书稿并发布。

## 添加新书

```bash
# 复制 book-2 模板即可
cp -r books/book-2 books/book-3
